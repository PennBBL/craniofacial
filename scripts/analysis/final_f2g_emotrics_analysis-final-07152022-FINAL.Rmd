...
```{r}
demo_szcr = merge(df %>% select(bbl_id), 
                  sz_cr %>% select(bbl_id, group)) %>%
  filter(group %in% c('SZ', 'CR')) %>%
  group_by(group) %>%
  count() %>%
  spread(group, n) %>%
  mutate(group = 'PS', "SZ / CR" = paste(SZ, CR, sep = ' / ')) %>%
  select(group, 'SZ / CR')
...
demo_race = df %>% 
  mutate(race = sapply(race, function(x) {
  ...
  count() %>%
  spread(race, n) %>%
  mutate("Black/African American / White / Other" = paste(`Black/African American`, 
                                                         White,
                                                         Other,
                                                         sep = " / ")) %>%
...
demo_fsiq = df %>%
  group_by(group) %>%
  summarise(FSIQ = sprintf("%.3f \U00B1 %.3f",
...
demo_del = demo_del %>% spread(`A-D Present`, n) %>%
  ...
  mutate('Full A-D Deletion / Partial A-D Deletion' = paste(`Full A-D Deletion`, 
                                                            `Partial A-D Deletion`,
                                                            sep = " / ")) %>%
  filter(group == '22q') %>%
  select(group, 'Full A-D Deletion / Partial A-D Deletion')
...
demo_gaf_mmse = df %>%
  group_by(group) %>%
  summarise(GAF= sprintf("%.0f \U00B1 %.0f",
...
caption = paste0("Out of ", demo_per$total[demo_per$group == '22q'], 
                 " 22q subjects, ", demo_per$age[demo_per$group == '22q'],
                 ...
                 #demo_per$FSIQ[demo_per$group == '22q'],
                 #" were missing FSIQ data, and ",
                 #demo_per$PS[demo_per$group == '22q'],
                 #" were missing PS data.",
...
demo_table2 = kable(demo_table,
  ...
  footnote(general = caption,
           general_title = "Table")
save_kable(demo_table2, file = file.path(project, 'tables', 'demo_table.html'))
webshot::webshot(file.path(project, 'tables', 'demo_table.html'), 
        file.path(project, 'tables', 'demo_table.pdf'))
...
pdf(file='~/Library/CloudStorage/Box-Box/Craniofacial Study/final_emotrics_f2g_analysis/manuscript_analyses/Figure1a.pdf')
fig1a
dev.off()
...
pdf(file='~/Library/CloudStorage/Box-Box/Craniofacial Study/final_emotrics_f2g_analysis/manuscript_analyses/SupplementalFigure1.pdf', 10,10)
SupplementalFigure1
dev.off()
```
### AD Group
...
pdf(file='~/Library/CloudStorage/Box-Box/Craniofacial Study/final_emotrics_f2g_analysis/manuscript_analyses/SupplementalFigure_22q_ADonly.pdf', 10,10)
...
pdf(file='~/Library/CloudStorage/Box-Box/Craniofacial Study/final_emotrics_f2g_analysis/manuscript_analyses/SupplementalFigure_22q_NONADonly.pdf', 10,10)
...
pdf(file='~/Library/CloudStorage/Box-Box/Craniofacial Study/final_emotrics_f2g_analysis/manuscript_analyses/SupplementalFigure_PS.pdf', 10,10)
...
pdf(file='~/Library/CloudStorage/Box-Box/Craniofacial Study/final_emotrics_f2g_analysis/manuscript_analyses/SupplementalFigure_TD.pdf', 10,10)
...
pdf(file='~/Library/CloudStorage/Box-Box/Craniofacial Study/final_emotrics_f2g_analysis/manuscript_analyses/22qGestaltROC.pdf')
...
```
```{r}
pdf(file='~/Library/CloudStorage/Box-Box/Craniofacial Study/final_emotrics_f2g_analysis/manuscript_analyses/Scree.pdf')
fviz_screeplot(resPCA) + theme(plot.title = element_text(hjust = 0.5))
dev.off()
```
## Loadings
...
#### PC1
```{r, fig.width=12, fig.height=8}
pdf(file='~/Library/CloudStorage/Box-Box/Craniofacial Study/final_emotrics_f2g_analysis/manuscript_analyses/PC1.pdf',10,10)
...
pdf(file='~/Library/CloudStorage/Box-Box/Craniofacial Study/final_emotrics_f2g_analysis/manuscript_analyses/PC2.pdf',10,10)
...
```{r, fig.width=12, fig.height=8}
pdf(file='~/Library/CloudStorage/Box-Box/Craniofacial Study/final_emotrics_f2g_analysis/manuscript_analyses/PC3.pdf',10,10)
...
```{r, fig.width=12, fig.height=8}
pdf(file='~/Library/CloudStorage/Box-Box/Craniofacial Study/final_emotrics_f2g_analysis/manuscript_analyses/PC4.pdf',10,10)
...
qPS_Vs_22qNOPS <- ggplot(patho_df, 
                         aes(x = finalPScat,
                             y = PC2,
                             fill = finalPScat)) +
  geom_boxplot_pattern(pattern_color =  c("white", "pink"),
...
#print plot
```{r}
pdf(file='~/Library/CloudStorage/Box-Box/Craniofacial Study/final_emotrics_f2g_analysis/manuscript_analyses/22q+_vs_22q-.pdf',10,10)
qPS_Vs_22qNOPS
dev.off()
```
...
```
```{r}
pdf(file='~/Library/CloudStorage/Box-Box/Craniofacial Study/final_emotrics_f2g_analysis/manuscript_analyses/22q+_vs_22q-_PC1PC2.pdf',10,10)
PC1_vs_PC2_22qPS_vs_22qnoPS
dev.off()
```
...
```
```{r}
pdf(file='~/Library/CloudStorage/Box-Box/Craniofacial Study/final_emotrics_f2g_analysis/manuscript_analyses/SuppFigPC1PC2.pdf',10,10)
supfigx
dev.off()
```
...
```
```{r}
pdf(file='~/Library/CloudStorage/Box-Box/Craniofacial Study/final_emotrics_f2g_analysis/manuscript_analyses/SuppFigPC2PC3.pdf',10,10)
supfigy
dev.off()
```
...
```{r}
pdf(file='~/Library/CloudStorage/Box-Box/Craniofacial Study/final_emotrics_f2g_analysis/manuscript_analyses/SuppFigPC3PC4.pdf',10,10)
supfigz
dev.off()
```
...
```
```{r}
pdf(file='~/Library/CloudStorage/Box-Box/Craniofacial Study/final_emotrics_f2g_analysis/manuscript_analyses/PCscoresByGroup.pdf',10,10)
a
dev.off()
```
...
PC3 of TD and CR after adjusting for age and sex is significant.
...
PC4 of TD and CR after adjusting for age and sex is not significant.
...
# get group N
group_n = emotrics[, c('bbl_id', 'group')]
group_n %<>% 
  group_by(group) %>%
  dplyr::count()
...
save_kable(emotrics_average, 
           file = file.path(project, 'tables', 'emotrics_average.html'))
webshot::webshot(file.path(project, 'tables', 'emotrics_average.html'), 
                 file.path(project, 'tables', 'emotrics_average.pdf'))
...
pdf(file='~/Library/CloudStorage/Box-Box/Craniofacial Study/final_emotrics_f2g_analysis/manuscript_analyses/Figure3.pdf')
...
for (r in 1:length(rois)){
  ...
  roi_data <- roi_df[, c('group', 'bbl_id', roi)] %>%
    group_by(group) %>%
    mutate(outlier = ifelse(is_outlier(get(roi)), bbl_id, as.numeric(NA)))
...
plt_list <- list()
pdf(file='~/Library/CloudStorage/Box-Box/Craniofacial Study/final_emotrics_f2g_analysis/manuscript_analyses/Figure4.pdf')
for (r in 1:length(rois)){
  roi = rois[r]
  roi_data <- chop_roi_df[, c('PS', roi)]
  roi_data = drop_na(roi_data)
  roi_plot <- ggplot(roi_data, aes_string(x = 'PS',
                                          y = roi,
                                          fill = 'PS')) +
    geom_boxplot_pattern(pattern_color =  c("white", "pink"),
                         pattern_fill = c("white", "pink"),
...
#plot all ROC
pdf(file='~/Library/CloudStorage/Box-Box/Craniofacial Study/final_emotrics_f2g_analysis/manuscript_analyses/ROC_PS_TD.pdf')
